...
function SwiftBara:CreateCategory(config)
    ...
    function Category:CreateModule(config)
        ...
        function Module:AddMode(cfg)
            ...
            local Mode = {Value = default, Options = options, Index = 1}
            ...
            Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = modeContainer})
            ...
            local modeText = Create("TextLabel", {
                Name = "Text",
            ...
            -- Hàm cập nhật mode hiển thị
            local function updateMode()
                modeText.Text = options[Mode.Index]
                Mode.Value = options[Mode.Index]
                callback(Mode.Value)
            end
            
            -- Xử lý cuộn chuột
            modeContainer.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    -- Nếu nhấn vào vùng trung tâm, chuyển sang mode tiếp theo
                    Mode.Index = (Mode.Index % #options) + 1
                    updateMode()
                end
            end)
            
            modeContainer.MouseWheelForward:Connect(function()
                Mode.Index = Mode.Index - 1
                if Mode.Index < 1 then Mode.Index = #options end
                updateMode()
            end)
            
            modeContainer.MouseWheelBackward:Connect(function()
                Mode.Index = Mode.Index + 1
                if Mode.Index > #options then Mode.Index = 1 end
                updateMode()
            end)
            ...
            prevBtn.MouseEnter:Connect(function()
                Tween(prevBtn, {TextColor3 = Theme.Primary}, 0.1)
            end)
            prevBtn.MouseLeave:Connect(function()
                Tween(prevBtn, {TextColor3 = Theme.TextDim}, 0.1)
            end)
            
            nextBtn.MouseEnter:Connect(function()
                Tween(nextBtn, {TextColor3 = Theme.Primary}, 0.1)
            end)
            nextBtn.MouseLeave:Connect(function()
                Tween(nextBtn, {TextColor3 = Theme.TextDim}, 0.1)
            end)
            
            modeContainer.MouseEnter:Connect(function()
                Tween(modeContainer, {BackgroundColor3 = Theme.Hover}, 0.1)
            end)
            modeContainer.MouseLeave:Connect(function()
                Tween(modeContainer, {BackgroundColor3 = Color3.fromRGB(38, 38, 52)}, 0.1)
            end)
