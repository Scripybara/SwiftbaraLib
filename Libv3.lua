...
-- Thêm function này sau các function Module:AddSlider, Module:AddToggle, Module:AddMode

function Module:AddColorPicker(cfg)
    cfg = cfg or {}
    local colorName = cfg.Name or "Color"
    local default = cfg.Default or Color3.fromRGB(255, 255, 255)
    local callback = cfg.Callback or function() end
    
    local Color = {Value = default}
    table.insert(Module.Settings, Color)
    ...
    Create("UICorner", {CornerRadius = UDim.new(0, 8), Parent = pickerPopup})
    ...
    Create("UICorner", {CornerRadius = UDim.new(0, 8), Parent = pickerHeader})
    ...
    closeBtn.MouseButton1Click:Connect(function()
        Tween(pickerPopup, {Size = UDim2.new(0, 0, 0, 0)}, 0.2)
        task.wait(0.2)
        pickerPopup.Visible = false
    end)
    
    closeBtn.MouseEnter:Connect(function()
        Tween(closeBtn, {TextTransparency = 0}, 0.1)
    end)
    
    closeBtn.MouseLeave:Connect(function()
        Tween(closeBtn, {TextTransparency = 0.3}, 0.1)
    end)
    
    -- Make draggable
    MakeDraggable(pickerPopup, pickerHeader)
    ...
    Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = spectrum})
    Create("UIStroke", {
        Parent = spectrum,
        Color = Color3.fromRGB(60, 60, 75),
        Thickness = 1
    ...
    Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = spectrumCursor})
    Create("UIStroke", {
    ...
    Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = hueSliderBg})
    ...
    local rgbFrame = Create("Frame", {
        ...
        Parent = pickerPopup,
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 82, 0, 130),
        Size = UDim2.new(0, 126, 0, 60)
    })
    ...
    local gInput = createRGBInput("G", UDim2.new(0, 44, 0, 0))
    local bInput = createRGBInput("B", UDim2.new(0, 88, 0, 0))
    ...
    -- Function to convert Color3 to RGB and Hex
    ...
    local function updateColor(color)
        ...
        -- Update text
        local r, g, b = toRGB(color)
        colorText.Text = string.format("RGB(%d, %d, %d)", r, g, b)
    ...
    local function updateFromColor(color)
        ...
        local x = s
        local y = 1 - v
        spectrumCursor.Position = UDim2.new(x, -3, y, -3)
    ...
    local function getColorFromPos(x, y)
        ...
        -- Get hue from slider
        local hue = hueHandle.Position.Y.Scale
        
        -- Saturation and value from spectrum
        local sat = x
        local val = 1 - y
    ...
    spectrum.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            local x = (input.Position.X - spectrum.AbsolutePosition.X) / spectrum.AbsoluteSize.X
            local y = (input.Position.Y - spectrum.AbsolutePosition.Y) / spectrum.AbsoluteSize.Y
            updateColor(getColorFromPos(x, y))
        end
    end)
    ...
    hueSliderBg.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            hueDragging = true
            local y = (input.Position.Y - hueSliderBg.AbsolutePosition.Y) / hueSliderBg.AbsoluteSize.Y
            local color = getColorFromPos(spectrumCursor.Position.X.Scale, 1 - spectrumCursor.Position.Y.Scale)
            updateColor(Color3.fromHSV(math.clamp(y, 0, 1), color.S, color.V))
        end
    end)
    ...
    rInput.FocusLost:Connect(function()
        rInput.Text = validateNumber(rInput.Text)
        updateFromRGB()
    end)
    
    gInput.FocusLost:Connect(function()
        gInput.Text = validateNumber(gInput.Text)
        updateFromRGB()
    end)
    
    bInput.FocusLost:Connect(function()
        bInput.Text = validateNumber(bInput.Text)
        updateFromRGB()
    end)
    ...
    colorBtn.MouseButton1Click:Connect(function()
        ...
        Tween(pickerPopup, {Size = UDim2.new(0, 220, 0, 180)}, 0.2)
        
        -- Position near the button
        local btnPos = colorBtn.AbsolutePosition
        local screenSize = workspace.CurrentCamera.ViewportSize
        
        local x = btnPos.X - 110
        local y = btnPos.Y + 25
        
        if x < 10 then x = 10 end
        if x + 220 > screenSize.X then x = screenSize.X - 230 end
        if y + 180 > screenSize.Y then y = screenSize.Y - 190 end
        
        pickerPopup.Position = UDim2.new(0, x, 0, y)
    ...
    function Color:Set(val)
        if typeof(val) == "Color3" then
            updateColor(val)
        end
    end
    
    function Color:Get() return Color.Value end
